...
const LocationDetailCard: React.FC<Props> = memo(({ location, onClose }) => {
  ...
  // จัดการรูปภาพ (ต่อ URL ให้ถ้าเป็นชื่อไฟล์สั้นๆ)
    ...
    <>
                ...
                {images.length > 1 && (
                  <>
                    <button
                      ...
                      className="absolute left-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/50 text-white hidden md:block"
                    >
                      <ChevronLeft size={24} />
                    </button>
                    <button
                      ...
                      className="absolute right-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/50 text-white hidden md:block"
                    >
                      ...
                      <div className="absolute left-6 right-6 top-[calc(100%-5px)] mt-2 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-20 max-h-52 overflow-y-auto custom-scrollbar">
                        {departments.map((dept, index) => {
                          const count =
                            dept === "ทั้งหมด"
                              ? allTeachers.length
                              : allTeachers.filter((t) => t.department === dept)
                                  .length;
                          ...
                          return (
                            ...
                            >
                              <span>
                                {dept === "ทั้งหมด" ? "แสดงทั้งหมด" : dept}
                              </span>
                              <span
                                className={`text-[10px] px-2 py-0.5 rounded-full ${isSelected ? "bg-white text-blue-600" : "bg-slate-100 text-slate-400"}`}
                              >
                                {count}
                              </span>
                            </button>
      ...
      {lightboxIndex !== null && (
        ...
        >
          <button
            onClick={closeLightbox}
            className="absolute top-6 right-6 p-3 bg-white/10 text-white rounded-full z-50"
          >
            <X size={24} />
          </button>
            ...
            {images.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-2 p-3 bg-white/10 text-white rounded-full"
                >
                  <ChevronLeft size={24} />
                </button>
                <button
                  onClick={showNextImage}
                  className="absolute right-2 p-3 bg-white/10 text-white rounded-full"
                >
                  <ChevronRight size={24} />
                </button>
              </>
